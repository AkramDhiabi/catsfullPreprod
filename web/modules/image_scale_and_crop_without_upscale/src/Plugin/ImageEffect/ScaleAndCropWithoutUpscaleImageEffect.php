...
namespace Drupal\image_scale_and_crop_without_upscale\Plugin\ImageEffect;
 ...
 *   label = @Translation("Scale and crop (without upscale)"),
 ...
 * )
 */
class ScaleAndCropWithoutUpscaleImageEffect extends ScaleAndCropImageEffect {
  ...
  private function adjustTargetDimensionsToPreventUpscale($sourceWidth, $sourceHeight) {
    ...
    elseif ($sourceWidth <= $targetWidth && $sourceHeight <= $targetHeight) {
      ...
      // the calculated source and target ratios may not be precisly the same
  ...
  public function transformDimensions(array &$dimensions, $uri) {
    if (!empty($dimensions['width']) && !empty($dimensions['height'])) {
      ...
      // We cannot accurately caculate the target dimensions if we don't already
